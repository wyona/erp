...
public class ERP {
	...
	public void addTask(String workspaceName, String title, String ownerID,
		...
		if (!existsOwner(workspaceName, owner)) {
			log
					.warn("No such owner: "
							+ owner
							+ " - Adding task aborted - You might want to add a person first (--help).");
			throw new ERPException(
					"No such owner: "
							+ owner
							+ " - Adding task aborted - You might want to add a person first.");
		}
		...
		if (projectID != null) {
			...
			if (!existsProject(workspaceName, project)) {
				log.warn("No such project: " + project
						+ " - Adding task aborted.");
				throw new ERPException("No such project: " + project
						+ " - Adding task aborted.");
			}
		}
		...
		try {
			...
			if (!tasksNode.hasNode(relPath)) {
				...
				Node ownerNode = rootNode.getNode(PERSONS_NODE_NAME + "/"
						+ owner.getID());
				createBidirectionalAssociation(taskNode, "task", ownerNode,
						"owner");

				// Create association with project
				if (project != null) {
					Node projectNode = rootNode.getNode(PROJECTS_NODE_NAME
							+ "/" + project.getID());
					createBidirectionalAssociation(taskNode, "task",
							projectNode, "project");
				}
	 ...
	 *            Title of project
	 * @param customer
	 *            Customer associated with project
	 * @throws ERPException
	 */
	public void addProject(String workspaceName, String id, String title,
			String customerID) throws ERPException {
		...
		if (customerID != null) {
			...
			if (!existsCompany(workspaceName, customer)) {
				log.warn("No such customer: " + customer
						+ " - Adding project aborted.");
				throw new ERPException("No such customer: " + customer
						+ " - Adding project aborted.");
			}
		}
		...
		try {
			...
			if (!projectsNode.hasNode(relPath)) {
				...
				if (customer != null) {
					Node customerNode = rootNode.getNode(COMPANIES_NODE_NAME
							+ "/" + customer.getID());
					createBidirectionalAssociation(projectNode, "project",
							customerNode, "customer");
				}

				session.checkPermission("/" + PROJECTS_NODE_NAME + relPath,
						"add_node");
				session.save();
			} else {
				log.info("Node already exists: " + relPath);
	...
	public void addProject(String workspaceName, String id, String title)
		 ...
		 * Project project = new Project(id, title);
		 * 
		 * Session session = null; try { Repository repo = getRepository();
		 * Credentials credentials = new SimpleCredentials(USERID, PASSWORD);
		 * session = repo.login(credentials, workspaceName); Node rootNode =
		 * session.getRootNode();
		 * 
		 * Node projectsNode = getNode(rootNode, PROJECTS_NODE_NAME);
		 * 
		 * String relPath = id; if (!projectsNode.hasNode(relPath)) { Node
		 * projectNode = projectsNode.addNode(relPath);
		 * projectNode.addMixin("mix:referenceable");
		 * projectNode.setProperty("title", title); log.info("UUID of project
		 * node: " + projectNode.getUUID()); log.info("Name of project node: " +
		 * projectNode.getName()); log.info("Path of project node: " +
		 * projectNode.getPath()); session.checkPermission("/"
		 * +PROJECTS_NODE_NAME + relPath, "add_node"); session.save(); } else {
		 * log.info("Node already exists: " + relPath); } } catch (Exception e) {
		 * log.error(e); } finally { if (session != null) session.logout(); }
		 */
	}
	 ...
	 * 
	 * @param id
	 *            ID of person, e.g. michi
	 * @param name
	 *            Name of person, e.g. Michael Wechner
	 * @throws ERPException
	 */
	public void addPerson(String workspaceName, String id, String name,
			String email) throws ERPException {
		log
		...
		try {
			...
			if (!personsNode.hasNode(relPath)) {
				...
				session.checkPermission("/" + PERSONS_NODE_NAME + relPath,
						"add_node");
				session.save();
			} else {
				log.info("Node already exists: " + relPath);
	...
	public void addCustomer(String workspaceName, String id, String name)
		...
		try {
			...
			if (!companiesNode.hasNode(relPath)) {
				...
				session.checkPermission("/" + COMPANIES_NODE_NAME + relPath,
						"add_node");
				session.save();
			} else {
				log.info("Node already exists: " + relPath);
	...
	public void addInvoice(String workspaceName, String customerID)
		...
		if (!existsCompany(workspaceName, customer)) {
			log
					.warn("No such customer: "
							+ customer
							+ " - Adding task aborted - You might want to add a customer first (--help).");
			throw new ERPException(
					"No such customer: "
							+ customer
							+ " - Adding task aborted - You might want to add a customer first (--help).");
		...
		try {
			...
			if (!invoicesNode.hasNode(relPath)) {
				...
				Node customerNode = rootNode.getNode(COMPANIES_NODE_NAME + "/"
						+ customer.getID());
				createBidirectionalAssociation(invoiceNode, "invoice",
						customerNode, "customer");
				session.checkPermission("/" + INVOICES_NODE_NAME + relPath,
						"add_node");
				session.save();
			} else {
				log.info("Node already exists: " + relPath);
	...
	public Hashtable listBean(String workspaceName, String relPath,
		...
		try {
			...
			if (typeNode != null) {
				...
				throw new ERPException("No such node: " + relPath);
			}
		} catch (PathNotFoundException e) {
			log.warn("No such path: " + relPath);
			throw new ERPException("No such path: " + relPath);
		} catch (Exception e) {
			log.error(e);
		...
		try {
			...
			if (typeNode != null) {
				...
				while (nit.hasNext()) {
					Node instanceNode = nit.nextNode();
					log.info("");
					log.info("UUID of " + typeName + " node: "
							+ instanceNode.getUUID());
					log.info("Name of " + typeName + " node: "
							+ instanceNode.getName());
					log.info("Path of " + typeName + " node: "
							+ instanceNode.getPath());

					// List properties
					PropertyIterator propprit = instanceNode.getProperties();
					while (propprit.hasNext()) {
						Property prop = propprit.nextProperty();
						log.info("Property of "
								+ typeName
								+ " node: "
								+ prop.getName()
								+ " , "
					...
					if (instanceNode.hasNode("associations")) {
						log.info("Node " + instanceNode.getPath()
								+ " has associations:");
						PropertyIterator prit = instanceNode.getNode(
								"associations").getProperties();
						while (prit.hasNext()) {
							Property prop = prit.nextProperty();
							log.info("Association of " + typeName + " node: "
									+ prop.getName());
							if (prop.getDefinition().isMultiple()) {
								log.info("Property has multiple values: "
										+ prop.getName());
						...
						log.info("Node " + instanceNode.getPath()
								+ " has no associations yet.");
					}

					// List references
					PropertyIterator prit = instanceNode.getReferences();
					if (!prit.hasNext())
						log.info("Node (" + instanceNode.getName()
								+ ") has no references.");
					while (prit.hasNext()) {
						Property prop = prit.nextProperty();
						log.info("Reference of " + typeName + " node: "
								+ prop.getName() + ", "
								+ prop.getNode().getPath());
					}
				}
			} else {
				log.warn("No such node: " + relPath);
	...
	public boolean existsCompany(String workspaceName, Customer customer) {
		return exists(workspaceName, COMPANIES_NODE_NAME + "/"
				+ customer.getID());
	}
	 ...
	 * 
	 */
	private void createBidirectionalAssociation(Node node1, String role1,
			Node node2, String role2) throws ERPException {
	 ...
	 * 
	 */
	private void updateValues(Node associations2, String role1,
			Node associatedNode) throws Exception {
	...
	public String getTopicMap(String workspaceName) {
		...
		sb
				.append("<topicMap xmlns=\"http://www.topicmaps.org/xtm/1.0/\" xmlns:xlink=\"http://www.w3.org/1999/xlink\">");
		...
		sb
				.append("<baseName><baseNameString>Project</baseNameString></baseName>");
		sb.append("</topic>");

		sb.append("<topic id=\"person\">");
		sb
				.append("<baseName><baseNameString>Person</baseNameString></baseName>");
		sb.append("</topic>");

		sb.append("<topic id=\"company\">");
		sb
				.append("<baseName><baseNameString>Company</baseNameString></baseName>");
		sb.append("</topic>");

		sb.append("<topic id=\"invoice\">");
		sb
				.append("<baseName><baseNameString>Invoice</baseNameString></baseName>");
		sb.append("</topic>");
